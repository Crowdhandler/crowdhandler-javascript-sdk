...
exports.createError = exports.ErrorCodes = exports.CrowdHandlerError = void 0;
/**
...
exports.createError = {
    apiConnection: function (originalError) {
        ...
        if (isTimeout) {
            return new CrowdHandlerError(exports.ErrorCodes.API_TIMEOUT, 'Request to CrowdHandler API timed out', 'Try increasing the timeout option or check your network connection', undefined, { originalError: originalError.message });
        }
        if (isRefused) {
            return new CrowdHandlerError(exports.ErrorCodes.API_CONNECTION_FAILED, 'Could not connect to CrowdHandler API', 'Check your internet connection and firewall settings. If the problem persists, check https://status.crowdhandler.com', undefined, { originalError: originalError.message });
        }
        return new CrowdHandlerError(exports.ErrorCodes.API_CONNECTION_FAILED, "Network error: ".concat(originalError.message), 'Check your network connection and try again', undefined, { originalError: originalError.message });
    },
    invalidApiKey: function (keyType) {
        var code = keyType === 'public' ? exports.ErrorCodes.INVALID_PUBLIC_KEY : exports.ErrorCodes.INVALID_PRIVATE_KEY;
    ...
    missingPrivateKey: function (method) {
        return new CrowdHandlerError(exports.ErrorCodes.MISSING_PRIVATE_KEY, "".concat(method, " requires a private key"), "Initialize with: crowdhandler.init({ publicKey, privateKey })", 403);
    },
    missingContext: function (method) {
        ...
        return new CrowdHandlerError(exports.ErrorCodes.MISSING_CONTEXT, "".concat(method, " requires request context"), "Initialize with: ".concat(example), 400);
    },
    invalidResponse: function (response) {
        return new CrowdHandlerError(exports.ErrorCodes.API_INVALID_RESPONSE, 'Received invalid response from CrowdHandler API', 'This might be a temporary issue. If it persists, contact support@crowdhandler.com', 502, { response: JSON.stringify(response).substring(0, 200) });
    },
    rateLimited: function (retryAfter) {
        return new CrowdHandlerError(exports.ErrorCodes.RATE_LIMITED, 'API rate limit exceeded', retryAfter
    ...
    resourceNotFound: function (resourceType, resourceId) {
        var errorCode = {
            domain: exports.ErrorCodes.DOMAIN_NOT_FOUND,
            room: exports.ErrorCodes.ROOM_NOT_FOUND,
            session: exports.ErrorCodes.SESSION_NOT_FOUND,
        }[resourceType] || exports.ErrorCodes.API_INVALID_RESPONSE;
